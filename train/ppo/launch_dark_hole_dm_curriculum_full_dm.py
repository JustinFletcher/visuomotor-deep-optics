#!/usr/bin/env python
"""Launch the 6 inner-ring full-DM (no-bilateral) curriculum runs.

Companion to ``launch_dark_hole_dm_curriculum.py``. Same six inner-
ring targets, same curriculum schedule, but uses the full 1240-DOF
action space and unmasked observation -- the bilateral wrapper is
off entirely. Also bumps entropy regulation (ent_coef 1e-5,
log_std_max -1.5) to keep the policy from converging onto a brittle
low-entropy pattern through the curriculum transition.

Usage:
    python train/ppo/launch_dark_hole_dm_curriculum_full_dm.py
    python train/ppo/launch_dark_hole_dm_curriculum_full_dm.py --target-id 0
    python train/ppo/launch_dark_hole_dm_curriculum_full_dm.py --illustrate
    python train/ppo/launch_dark_hole_dm_curriculum_full_dm.py --local
"""
import argparse
import os
import secrets
...
_TRAIN_SCRIPT = "train/ppo/train_ppo_elf_dm_dark_hole_curriculum_full_dm.py"
_RUN_PREFIX = "dark_hole_dm_curriculum_full"


def make_sbatch_script(target_idx, run_id, run_dir_base, seed,
                       angle_deg, radius_frac, size_radius,
                       curriculum_warmup, curriculum_anneal,
                       ent_coef, log_std_max,
                       wall_time=SLURM_TIME):
    run_dir = os.path.join(run_dir_base, f"target_{target_idx:02d}")
    job_name = f"dhf-{run_id[-8:]}-{target_idx:02d}"
    extra = ""
    if curriculum_warmup is not None:
        extra += f" \\\n            --curriculum-warmup {int(curriculum_warmup)}"
    if curriculum_anneal is not None:
        extra += f" \\\n            --curriculum-anneal {int(curriculum_anneal)}"
    if ent_coef is not None:
        extra += f" \\\n            --ent-coef {float(ent_coef)}"
    if log_std_max is not None:
        extra += f" \\\n            --log-std-max {float(log_std_max)}"
...
def render_illustration(targets, out_path: Path, only_ids):
    ...
    from train.ppo.train_ppo_elf_dm_dark_hole_curriculum_full_dm import ENV_KWARGS

    kw = dict(ENV_KWARGS)
    kw["dark_hole"] = False
    kw["init_dm_micron_std"] = 0.0
    kw["silence"] = True
    ...
    ax.set_title(
        "Full-DM curriculum coverage (inner ring, no bilateral)\n"
        "cyan: target  no blind region (wrapper off)",
        fontsize=9)
...
def main():
    parser = argparse.ArgumentParser(
        description="Launch the 6 inner-ring full-DM curriculum runs.")
    ...
    parser.add_argument("--curriculum-warmup", type=int, default=None)
    parser.add_argument("--curriculum-anneal", type=int, default=None)
    parser.add_argument(
        "--ent-coef", type=float, default=None,
        help="Override entropy coefficient (default: 1e-5 from script).")
    parser.add_argument(
        "--log-std-max", type=float, default=None,
        help="Override per-dim log_std cap (default: -1.5 from script).")
    parser.add_argument("--illustrate", action="store_true")
    parser.add_argument(
        "--illustrate-out", type=str,
        default="dark_hole_runs/dark_hole_dm_curriculum_full_coverage")
    ...
    if cli.illustrate:
        render_illustration(targets, Path(cli.illustrate_out), only_ids=inner_ids)
        return
    ...
    print(f"Bilateral:    OFF (full 1240-DOF action space)")
    print(f"Entropy:      ent_coef={cli.ent_coef or '1e-5 (default)'}, "
          f"log_std_max={cli.log_std_max or '-1.5 (default)'}")
    print(f"Curriculum:   warmup={cli.curriculum_warmup or '1,000,000 (default)'}, "
          f"anneal={cli.curriculum_anneal or '10,000,000 (default)'}")
    ...
    if cli.local:
        for i in target_indices:
            ...
            if cli.curriculum_warmup is not None:
                cmd.extend(["--curriculum-warmup", str(cli.curriculum_warmup)])
            if cli.curriculum_anneal is not None:
                cmd.extend(["--curriculum-anneal", str(cli.curriculum_anneal)])
            if cli.ent_coef is not None:
                cmd.extend(["--ent-coef", str(cli.ent_coef)])
            if cli.log_std_max is not None:
                cmd.extend(["--log-std-max", str(cli.log_std_max)])
    ...
    for i in target_indices:
        ...
        script = make_sbatch_script(
            ...
            seed=seeds[i], angle_deg=a, radius_frac=r, size_radius=s,
            curriculum_warmup=cli.curriculum_warmup,
            curriculum_anneal=cli.curriculum_anneal,
            ent_coef=cli.ent_coef, log_std_max=cli.log_std_max,
            wall_time=cli.time)
        if cli.dry_run:
