"""
PPO training: ELF piston-only alignment (15 DOF).

Trains a recurrent PPO agent to align piston offsets on the ELF
distributed-aperture telescope (15 segments, 1 piston DOF each).

Reward is distance-weighted (centered) Strehl, matching the nanoelf
PTT curriculum configuration.

All hyperparameters are embedded in this file so that a training run
is fully reproducible from this single artifact.

Usage:
    python train/ppo/train_ppo_elf_piston.py                          # local run (v4, 8 envs)
    python train/ppo/train_ppo_elf_piston.py --hpc                    # HPC run (v5, 64 envs, no eval)
    python train/ppo/train_ppo_elf_piston.py --hpc --no-eval          # HPC, skip eval
"""
...
ELF_PISTON_ENV_KWARGS = {
    ...
    "reward_weight_centered_strehl": 1.0,  # distance-weighted Strehl
...
LOCAL_CONFIG = dict(
    ...
    # --- Env kwargs ---
    env_kwargs=ELF_PISTON_ENV_KWARGS,
)
