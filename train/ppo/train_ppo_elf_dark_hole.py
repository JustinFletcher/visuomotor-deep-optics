"""
PPO training: ELF piston-only dark-hole shaping.

Trains a recurrent PPO agent to suppress focal-plane intensity in a
designated off-axis region (the dark hole) while maintaining good
on-axis alignment. The agent commands segment piston only (15 DOF).

Initial conditions are drawn to match the residual piston distribution
left by the bootstrap-composed alignment agent at the end of its
rollout — i.e., this script is intended as a *fine-tuning* stage,
operating in the regime where coarse alignment has already converged.

The dark-hole geometry (angle, radial location, size) is fixed for
the whole run. To launch a sweep across many randomly-chosen holes,
use ``train/ppo/launch_dark_hole.py``.

Usage:
    python train/ppo/train_ppo_elf_dark_hole.py \\
        --dark-hole-angle 45 \\
        --dark-hole-radius-frac 0.20 \\
        --dark-hole-size 0.04
    python train/ppo/train_ppo_elf_dark_hole.py --hpc --no-eval ...
"""
...
HPC_CONFIG = dict(
    ...
    # because the figures themselves were tightened to about a quarter
