#!/usr/bin/env python
"""Launch the 6 inner-ring dark-hole + curriculum runs.

One SLURM job per inner-ring target. Each runs
``train_ppo_elf_dm_dark_hole_curriculum.py``, which holds dark-hole
weight at zero for the first 1M steps (Strehl-only pre-training)
then linearly ramps it to 1.0 over the next 10M steps.

Same env / wrapper / reward layout as the strehl-only sanity check,
plus dark-hole geometry per target.

Usage:
    python train/ppo/launch_dark_hole_dm_curriculum.py
    python train/ppo/launch_dark_hole_dm_curriculum.py --target-id 2
    python train/ppo/launch_dark_hole_dm_curriculum.py --illustrate
    python train/ppo/launch_dark_hole_dm_curriculum.py --local
    python train/ppo/launch_dark_hole_dm_curriculum.py \\
        --curriculum-warmup 2000000 --curriculum-anneal 20000000
"""
import argparse
import os
import secrets
...
_TRAIN_SCRIPT = "train/ppo/train_ppo_elf_dm_dark_hole_curriculum.py"
_RUN_PREFIX = "dark_hole_dm_curriculum"
...
def render_illustration(targets, out_path: Path, only_ids):
    """Inner-ring targets + their fixed-vertical blind regions."""
    ...
    from train.ppo.train_ppo_elf_dm_dark_hole_curriculum import ENV_KWARGS
    ...
    for i, (angle, r, size) in enumerate(targets):
        ...
        # fixed_vertical mode -- blind is the horizontal mirror.
    ...
    ax.set_title(
        "Dark-hole DM curriculum coverage (inner ring)\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Launch the 6 inner-ring dark-hole + curriculum runs.")
    ...
    parser.add_argument(
        ...
        help="Override the dark-hole curriculum warmup window "
             "(default: 1,000,000 timesteps).")
    parser.add_argument(
        "--curriculum-anneal", type=int, default=None,
        help="Override the dark-hole curriculum anneal window "
             "(default: 10,000,000 timesteps).")
    parser.add_argument("--illustrate", action="store_true")
    parser.add_argument("--illustrate-out", type=str,
                        default="dark_hole_runs/dark_hole_dm_curriculum_coverage")
