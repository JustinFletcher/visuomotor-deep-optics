...
SLURM_TIME = "72:00:00"
SLURM_GRES = "gpu"
# The code (and read-only input checkpoints) live on the home
# filesystem; SLURM jobs `cd` here to execute. Override with
# VMDO_HPC_CODE_DIR if your clone lives somewhere else.
HPC_CODE_DIR = os.environ.get(
    "VMDO_HPC_CODE_DIR",
    "/p/home/fletch/visuomotor-deep-optics")

# Runtime *outputs* (training run dirs, checkpoints, fine-tune
# outputs) must land on the work filesystem per cluster policy
# ("jobs running on filesystems other than /p/work / $WORKDIR will
# be terminated"). Resolved at launcher-import time from $WORKDIR.
# Launchers default their --output-root under this; an explicit
# --output-root still wins. Override with VMDO_HPC_WORKDIR.
HPC_WORKDIR = os.environ.get(
    "VMDO_HPC_WORKDIR",
    os.environ.get("WORKDIR", "/p/work/fletch"))
