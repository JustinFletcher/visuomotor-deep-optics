"""
Optomech V3 -- High-performance Gymnasium environment for distributed-aperture
telescope control.

This is a performance-optimized version of optomech_v2.py with:
  - anytree dependency eliminated (replaced with list-based index mapper)
  - Wavelength list, normalized images, object spectrum, detector constants
    all pre-computed and cached
  - Vectorized mask creation (np.ogrid instead of Python double-loop)
  - scipy.fft with multi-threaded workers for FFT convolution
  - Local variable aliases in hot loops to minimize attribute lookups
  - Reward dispatch via dict lookup instead of if/elif chain
  - Pre-computed stroke limits and secondary correction constants

Functionally identical to v2 -- same action/observation spaces, same reward
...
# ===================================================================
# V3 Logging  (magenta tag to distinguish from v2 cyan)
# ===================================================================

_V3_TAG = "\033[95m[optomech-v3]\033[0m"
_V3_TAG_PLAIN = "[optomech-v3]"
...
class OpticalSystem(object):
    ...
    def _init_natural_diff_motion(self):
        ...
        if self.model_wind_diff_motion:
            ...
            wind_ptt[:, 1] = np.random.randn(self.num_apertures) * tip_std * np.pi / (180 * 3600)
            wind_ptt[:, 2] = np.random.randn(self.num_apertures) * tilt_std * np.pi / (180 * 3600)
...
class OptomechEnv(gym.Env):
    ...
    def step(self, action, noisy_command=False, reset=False):
        ...
        for frame_num in range(_frames_per_decision):
            frame = np.zeros(_image_shape, dtype=np.float32)
            ...
            for command_num in range(_cmds_per_frame):
                ...
                for ao_step in range(_ao_steps_per_cmd):
                    for wl in _wavelengths:
                        ...
                        science = _optical_sys.get_science_frame(
                            integration_seconds=integration_sec)
                        self.science_readout_raster = np.reshape(
                            science, _image_shape)

                        frame += self.science_readout_raster / float(len(_wavelengths))
        ...
        # quality ∈ [0, 1] (0 = bad, 1 = perfect) derived from the raw
        # reward (which is always ≤ 0, approaching 0 for good states).
        # stillness ∈ [0, 1] (1 = no movement, 0 = full action).
        # Bonus = weight × quality × stillness.
        # Only rewards holding when already in a good state; does not
        # penalize exploration when the state is poor.
        if self._holding_bonus_weight > 0:
            quality = float(np.clip(1.0 + reward, 0.0, 1.0))
    ...
    def _absolute_strehl(self, fpi):
        """Unit-independent Strehl: (max/sum)_obs / (max/sum)_perfect.

        Both images can be in different units (raw intensity vs DN) because
        the max/sum ratio is dimensionless.
        """
        fpi_sum = float(np.sum(fpi))
        if fpi_sum <= 0:
            return 0.0
        obs_peak_over_sum = float(np.max(fpi)) / fpi_sum
        return obs_peak_over_sum / self._perfect_peak_over_sum
    ...
    def _reward_factored(self):
        """Three-factor reward: shape + dark_hole + strehl.
