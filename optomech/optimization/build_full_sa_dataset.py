#!/usr/bin/env python3
"""
Full SA Dataset Builder - Complete Simulated Annealing Trajectory Recording

This script records the COMPLETE simulated annealing trajectory through configuration space,
including both accepted AND rejected transitions. Unlike build_sa_dataset.py which only saves
accepted transitions, this captures the exact path SA took including "backtracking" moves.

Key Differences from build_sa_dataset.py:
- Saves ALL transitions (accepted + rejected)
- For rejected candidates: records TWO steps:
  1. The forward step to the rejected candidate configuration
  2. The "return" step back to the prior accepted configuration (using prior action)
- Accurately represents SA's exploration through the configuration space

Dataset Structure:
Each saved sample contains:
- observation: State after the action was applied
- sa_action: Action that was taken (candidate for rejected, accepted for accepted)
- perfect_action: Ideal correcting action for this state
- sa_incremental_action: Incremental action from previous accepted state
- perfect_incremental_action: Ideal incremental correction from previous accepted state
- reward: Reward achieved
- next_observation: State after applying the action
- temperature: SA temperature when transition occurred
- cost_delta: Cost delta for this transition
- accepted: Boolean indicating if this was an accepted transition
- episode_id: UUID identifying which optimization episode
- episode_step: Sequential step number within the episode (includes ALL steps)
- optimization_step: Total SA optimization step counter

This dataset allows training models on:
1. The complete SA exploration strategy (including rejections)
2. Understanding why certain actions were rejected
3. Learning the "return to previous state" behavior after rejections

Usage:
    python build_full_sa_dataset.py --num-samples 10000 --dataset-name full_sa_dataset
    python build_full_sa_dataset.py --job-config-file custom_config.json --num-samples 50000
"""
...
class Args:
    """
    Command-line arguments for the full SA trajectory dataset builder.
    Captures ALL SA steps including rejected transitions.
    """
    # 1. Dataset Settings
    num_samples: int = 10000
    dataset_save_path: str = "./datasets/"
    dataset_name: str = "full_sa_dataset"
    chunked_dataset: bool = False
    chunk_size: int = 1000
    write_frequency: int = 100  # Write samples to disk every N total steps (not just accepted)
...
def cli_main(args):
    """
    Command-line interface entry point for the full SA trajectory dataset builder.
    """
    print("Parsed arguments:")
    for key, value in vars(args).items():
        print(f"{key}: {value}")

    build_full_sa_dataset(args)


def build_full_sa_dataset(args):
    """
    Main function to build the full SA trajectory dataset.
    Runs simulated annealing optimization and saves ALL transitions including:
    - Accepted transitions (forward moves)
    - Rejected candidate transitions (forward move + return move)
    """
    ...
    def write_batch(sample_pairs, global_sample_count):
        """Write a batch of ALL SA samples to HDF5 file with UUID naming for parallel safety"""
        if not sample_pairs:
            return None
        ...
        cost_deltas = np.array([pair['cost_delta'] for pair in sample_pairs], dtype=np.float32)
        accepted = np.array([pair['accepted'] for pair in sample_pairs], dtype=np.bool_)
        ...
        try:
            ...
            with h5py.File(batch_path, 'w') as f:
                ...
                f.create_dataset('cost_deltas', data=cost_deltas, compression='gzip', compression_opts=6)
                f.create_dataset('accepted', data=accepted, compression='gzip', compression_opts=6)
                ...
                f.attrs['dataset_type'] = 'full_sa_trajectory'
                ...
                f.attrs['description'] = f'Full SA trajectory batch {batch_uuid} ({len(sample_pairs)} samples, includes accepted+rejected)'
            ...
            np.savez_compressed(
                ...
                dataset_type='full_sa_trajectory',
                env_id=args.env_id,
    ...
    sample_pairs = []  # List of ALL SA transitions (accepted + rejected + return steps)
    ...
    print(f"Building full SA trajectory dataset with {samples_needed} additional samples...")
    print(f"⚠️  NOTE: This records ALL transitions including rejected steps!")
    print(f"Incremental write frequency: every {args.write_frequency} total steps")
    print("=" * 60)

    # Initialize SA variables
    action_size = env.action_space.shape[0]
    
    total_samples_saved = 0  # Total samples saved (including accepted + rejected + return steps)
    accepted_count = 0  # Count of accepted transitions
    rejected_count = 0  # Count of rejected transitions
    total_steps = 0
    ...
    print(f"Starting SA optimization to collect {samples_needed} total trajectory samples...")
    
    # Outer loop: run multiple episodes until we have enough samples
    while total_samples_saved < samples_needed:
        # Check periodically if another job has completed the target
        if total_samples_saved % 100 == 0 and total_samples_saved > 0:
        ...
        episode_step = 0  # Step counter within current episode (includes ALL steps)
        ...
        for step_in_episode in range(args.max_episode_steps - 1):  # -1 because we already took one step above
            ...
            if total_samples_saved >= samples_needed:
                break
            ...
            if (cost_delta <= 0.0) or (np.exp(-cost_delta / temperature) > random.uniform(0.0, 1.0)):
                ...
                # Store accepted transition
                obs_uint16 = np.clip(obs, 0, 65535).astype(np.uint16)
                next_obs_uint16 = np.clip(next_obs, 0, 65535).astype(np.uint16)
                sample_pair = {
                    'observation': obs_uint16.tolist(),
                    'sa_action': candidate_actions.tolist(),
                    'sa_incremental_action': sa_incremental_action.tolist(),
                    'perfect_incremental_action': perfect_incremental_action.tolist(),
                    'perfect_action': perfect_action.tolist(),
                    'reward': float(candidate_rewards),
                    'next_observation': next_obs_uint16.tolist(),
                ...
                # ==================== REJECTED TRANSITION ====================
                # Save TWO steps: forward to rejected candidate, then return to prior state
                rejected_count += 1
                
                # Compute incremental actions for rejected candidate
                if previous_accepted_action is not None:
                    sa_incremental_action = candidate_actions - previous_accepted_action
                    perfect_incremental_action = perfect_action - previous_accepted_action
                else:
                ...
                # Step 1: Forward move to rejected candidate configuration
                obs_uint16 = np.clip(obs, 0, 65535).astype(np.uint16)
                next_obs_uint16 = np.clip(next_obs, 0, 65535).astype(np.uint16)
                forward_sample = {
                    'observation': obs_uint16.tolist(),
                ...
                sample_pairs.append(forward_sample)
                total_samples_saved += 1
                episode_step += 1
                
                # Step 2: Return move back to prior accepted configuration
                # We need to step the environment with the prior action to get back
                return_obs, return_rewards, return_terminations, return_truncations, return_infos = env.step(previous_accepted_action)
                
                # Get perfect action for the state we're returning to
                return_perfect_action = get_perfect_correction_action(env.optical_system)
                
                # For the return step, incremental action is the prior action (relative to rejected state)
                # Since we're at rejected state and taking prior action to return
                return_sa_incremental = previous_accepted_action.copy() - candidate_actions.copy()  # Action taken to return
                return_perfect_incremental = previous_accepted_action.copy() - return_perfect_action.copy()
                
                return_obs_uint16 = np.clip(next_obs, 0, 65535).astype(np.uint16)  # Starting from rejected state
                return_next_obs_uint16 = np.clip(return_obs, 0, 65535).astype(np.uint16)  # Back to prior state
                return_sample = {
                    'observation': return_obs_uint16.tolist(),  # Rejected candidate state
                    'sa_action': actions.tolist(),  # Prior action (to return)
                    'sa_incremental_action': return_sa_incremental.tolist(),
                    'perfect_incremental_action': return_perfect_incremental.tolist(),
                    'perfect_action': return_perfect_action.tolist(),
                    'reward': float(return_rewards),
                    'next_observation': return_next_obs_uint16.tolist(),  # Back to prior accepted state
                    'temperature': float(temperature),
                    'cost_delta': float(-cost_delta),  # Negative of forward cost delta (going back)
                    'accepted': False,  # This is part of rejection handling
                    'episode_id': current_episode_id,
                    'episode_step': episode_step,
                    'optimization_step': total_steps
                }
                sample_pairs.append(return_sample)
                total_samples_saved += 1
                episode_step += 1
                
                # Update state: we're back at the prior accepted state
                obs = return_obs
                # actions remain the same (prior accepted action)
                # previous_accepted_action remains the same
            ...
            if len(sample_pairs) >= args.write_frequency:
                current_total_samples = existing_samples + total_samples_saved
                batch_id = write_batch(sample_pairs, current_total_samples)
                if batch_id:
                    batch_ids.append(batch_id)
                    print(f"💾 Wrote batch {len(batch_ids)} ({len(sample_pairs)} total samples) to disk: {batch_id}")
                
                # Clear batch list for next batch
                sample_pairs = []
            
            # Progress reporting
            if total_steps % 100 == 0 or total_samples_saved >= samples_needed:
                current_total_samples = existing_samples + total_samples_saved
                progress = current_total_samples / args.num_samples * 100
                current_time = time.time()
                elapsed = current_time - start_time
                samples_per_sec = total_samples_saved / elapsed if elapsed > 0 else 0
                remaining_needed = samples_needed - total_samples_saved
                ...
                print(f"Progress: {current_total_samples}/{args.num_samples} ({progress:.1f}%) | "
                      f"Total: {total_samples_saved}/{samples_needed} | Accepted: {accepted_count} | Rejected: {rejected_count} | "
                      f"Steps: {total_steps} | Rate: {samples_per_sec:.2f} samples/s | "
    ...
    if sample_pairs:
        final_total_samples = existing_samples + total_samples_saved
        batch_id = write_batch(sample_pairs, final_total_samples)
        if batch_id:
            batch_ids.append(batch_id)
            print(f"💾 Wrote final batch {len(batch_ids)} ({len(sample_pairs)} total samples) to disk: {batch_id}")

    # Get final count from directory
    final_sample_count = count_existing_samples()
    
    print(f"\n✅ Full SA trajectory dataset generation completed!")
    ...
    print(f"Accepted transitions: {accepted_count}")
    print(f"Rejected transitions: {rejected_count} (each = 2 samples: forward + return)")
    print(f"Acceptance rate: {accepted_count/(accepted_count+rejected_count)*100:.2f}%")
    ...
    print(f"Write frequency: {args.write_frequency} total samples per batch")
    print(f"Data format: HDF5 compressed with FULL SA trajectory (accepted+rejected+return)")
