#!/usr/bin/env python3
"""
Simple Background Job Manager for Direct SML Dataset Generation

Runs dataset generation jobs directly using threading.
...
from build_optomech_dataset import build_dataset, Args
...
def run_single_job(job_id: int, config: JobConfig, results: List):
    ...
    try:
        ...
        print(f"🚀 Starting job {job_id}: {job_samples} samples (total progress: {samples_start + job_samples}/{config.total_samples})")
        ...
        args = Args()
        args.num_samples = job_samples  # Use calculated job samples, not config.samples_per_job
        ...
        results.append({
            'job_id': job_id,
            'success': True,
            'samples_generated': job_samples,  # Use actual samples generated
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Simple Job Manager for Direct SML Dataset Generation")
    ...
    print("🎯 Simple SML Dataset Generation Job Manager")
    print("=" * 50)
    ...
    print("💡 Use 'poetry run python sml_job_watcher.py' to monitor progress")
    print("=" * 50)
