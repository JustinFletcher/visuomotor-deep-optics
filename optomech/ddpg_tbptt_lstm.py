...
class ImpalaActor(nn.Module):
    ...
    def forward(self,
        ...
        if sequence_input:
            ...
            # Our input is [S, B, frame, channels, height, width]
...
if __name__ == "__main__":
    ...
    for iteration in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            actions_batch_batch =     torch.tensor(np.array(actions_batch_batch), dtype=torch.float32, device=device)
            prior_actions_batch =     torch.tensor(np.array(prior_actions_batch), dtype=torch.float32, device=device)
            rewards_batch =           torch.tensor(np.array(rewards_batch), dtype=torch.float32, device=device)
            prior_rewards_batch =     torch.tensor(np.array(prior_rewards_batch), dtype=torch.float32, device=device)
            next_observations_batch = torch.tensor(np.array(next_observations_batch), dtype=torch.float32, device=device)
            dones_batch =             torch.tensor(np.array(dones_batch), dtype=torch.float32, device=device)
