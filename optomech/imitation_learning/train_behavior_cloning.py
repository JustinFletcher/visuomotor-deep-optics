...
def train_epoch(model: nn.Module, dataloader: DataLoader, optimizer: optim.Optimizer,
                criterion: nn.Module, device: torch.device, grad_clip: float = None) -> Tuple[float, Dict, np.ndarray, Dict, np.ndarray, np.ndarray, torch.Tensor]:
    """
    Train for one epoch and return loss, MAE metrics, raw error distribution, absolute error metrics,
    and data for MAE vs target L2 analysis, plus sample observations from first batch.
