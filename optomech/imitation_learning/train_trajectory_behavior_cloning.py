#!/usr/bin/env python3
"""
Trajectory-Based Behavior Cloning Training Script with LSTM and TBPTT

Trains a ResNet-18 + LSTM actor model to predict action sequences from trajectory observations.
Features:
- LSTM architecture for temporal modeling
- Truncated Backpropagation Through Time (TBPTT)
- Sequence-based learning from SA datasets
...
- Balanced sampling and dataset pruning support
"""
...
class TrainingConfig:
    ...
    model_save_path: str = "saved_models/bc_lstm_resnet18_model.pth"
    ...
    sequence_length: int = 20  # Length of trajectory sequences
    sequence_stride: int = 10  # Stride for sliding window (overlap between sequences)
    lstm_hidden_dim: int = 256  # Hidden dimension for LSTM layers
    lstm_num_layers: int = 1  # Number of LSTM layers
    lstm_dropout: float = 0.0  # Dropout between LSTM layers (only if num_layers > 1)
    tbptt_chunk_size: int = 20  # Chunk size for TBPTT (None = full sequence)
    
    # Training settings
    optimizer: str = "adam"
    weight_decay: float = 1e-5
    grad_clip: float = 1.0  # Important for LSTMs
    ...
    # Rollout settings
    enable_rollouts: bool = False  # Disabled by default for trajectory training (requires sequence handling)
...
def train_epoch(model: nn.Module, dataloader: DataLoader, optimizer: optim.Optimizer,
    ...
    Args:
        ...
        dataloader: DataLoader providing trajectory sequences
        optimizer: Optimizer
        criterion: Loss function
        device: Device to run on
        grad_clip: Gradient clipping value (None = no clipping)
        tbptt_chunk_size: Chunk size for TBPTT (None = full sequence)
    ...
    total_loss = 0.0
    num_batches = 0
    all_mae_errors = []  # Example-level MAE (mean across actions per sample per timestep)
    ...
    for batch_data in tqdm(dataloader, desc="Training", leave=False):
        ...
        if tbptt_chunk_size is None or tbptt_chunk_size >= seq_len:
            # Process full sequence
            hidden = None
            predictions, hidden = model(observations, hidden)  # [batch, seq_len, action_dim]
            loss = criterion(predictions, actions)
            
            # Apply mask to loss if needed (for padded sequences)
            if not masks.all():
                loss = (loss * masks.unsqueeze(-1)).sum() / masks.sum()
            
            loss.backward()
        else:
            # TBPTT: Process sequence in chunks
            hidden = None
            total_chunk_loss = 0.0
            num_chunks = 0
            
            for chunk_start in range(0, seq_len, tbptt_chunk_size):
                chunk_end = min(chunk_start + tbptt_chunk_size, seq_len)
                
                # Get chunk
                obs_chunk = observations[:, chunk_start:chunk_end, ...]  # [batch, chunk_len, ...]
                action_chunk = actions[:, chunk_start:chunk_end, :]  # [batch, chunk_len, action_dim]
                mask_chunk = masks[:, chunk_start:chunk_end]  # [batch, chunk_len]
                
                # Forward through chunk
                if hidden is not None:
                    # Detach hidden state to truncate backprop
                    hidden = tuple(h.detach() for h in hidden)
                
                pred_chunk, hidden = model(obs_chunk, hidden)
                chunk_loss = criterion(pred_chunk, action_chunk)
                
                # Apply mask if needed
                if not mask_chunk.all():
                    chunk_loss = (chunk_loss * mask_chunk.unsqueeze(-1)).sum() / mask_chunk.sum()
                
                # Accumulate loss
                total_chunk_loss += chunk_loss
                num_chunks += 1
            
            # Average loss across chunks and backward
            loss = total_chunk_loss / num_chunks
            loss.backward()
        ...
        # Calculate errors for metrics (use full sequence predictions)
        with torch.no_grad():
            if tbptt_chunk_size is not None and tbptt_chunk_size < seq_len:
                # Re-run forward pass to get full predictions (for metrics only)
                predictions, _ = model(observations, None)
            
            # Reshape for metrics: [batch, seq_len, action_dim] -> [batch*seq_len, action_dim]
            predictions_flat = predictions.view(-1, predictions.shape[-1])
            actions_flat = actions.view(-1, actions.shape[-1])
            masks_flat = masks.view(-1)
            
            # Only compute metrics for valid (non-masked) timesteps
            valid_predictions = predictions_flat[masks_flat]
            valid_actions = actions_flat[masks_flat]
            
            if len(valid_predictions) > 0:
                # Calculate raw errors
                raw_errors = valid_predictions - valid_actions
                all_raw_errors.extend(raw_errors.cpu().numpy().flatten())
                
                # Calculate absolute errors
                abs_errors = torch.abs(raw_errors)
                all_abs_errors.extend(abs_errors.cpu().numpy().flatten())
                
                # Calculate MAE for each timestep
                mae_per_timestep = torch.mean(abs_errors, dim=1)
                all_mae_errors.extend(mae_per_timestep.cpu().numpy())
                all_mae_per_sample.extend(mae_per_timestep.cpu().numpy())
                
                # Calculate L2 norm of target actions per timestep
                target_l2 = torch.norm(valid_actions, p=2, dim=1)
...
def validate_epoch(model: nn.Module, dataloader: DataLoader, 
    ...
    """
    Validate for one epoch with sequence-based model and return loss, MAE metrics, 
    raw error distribution, and absolute error metrics.
    ...
    total_loss = 0.0
    num_batches = 0
    all_mae_errors = []  # Example-level MAE (mean across actions per timestep)
    all_raw_errors = []  # All raw errors (predictions - targets) flattened
    all_abs_errors = []  # All absolute errors flattened
    all_target_l2_norms = []  # L2 norm of target actions for each timestep
    all_mae_per_sample = []  # MAE per timestep for candlestick plot
    ...
    with torch.no_grad():
        for batch_data in tqdm(dataloader, desc="Validation", leave=False):
            ...
            # Forward pass
            predictions, _ = model(observations, hidden=None)  # [batch, seq_len, action_dim]
            loss = criterion(predictions, actions)
            
            # Apply mask to loss if needed
            if not masks.all():
                loss = (loss * masks.unsqueeze(-1)).sum() / masks.sum()
            ...
            if len(valid_predictions) > 0:
                ...
                # Track L2 norm of target actions and MAE per timestep
                target_l2 = torch.norm(valid_actions, p=2, dim=1)
                all_target_l2_norms.extend(target_l2.cpu().numpy())
                all_mae_per_sample.extend(mae_per_timestep.cpu().numpy())
...
def train_behavior_cloning(config: TrainingConfig):
    ...
    # Load dataset using TrajectoryDataset for sequence-based learning
    print(f"\n📚 Loading trajectory dataset from {config.dataset_path}")
    ...
    print(f"  TBPTT chunk size: {config.tbptt_chunk_size if config.tbptt_chunk_size else 'Full sequence'}")
    ...
    print(f"✅ Loaded {len(full_dataset)} trajectory sequences")
    
    # Prune dataset by target action L2 norm if enabled
    # For trajectory dataset, we average L2 across the sequence
    if config.prune_dataset:
        print(f"\n✂️  Pruning trajectory dataset by average target action L2 norm...")
        ...
        # Compute average L2 norms for all sequences
        all_l2_norms = []
        for idx in range(len(full_dataset)):
            _, actions, _ = full_dataset[idx]  # Get action sequence
            # Compute L2 norm for each action in sequence, then average
            l2_norms_in_seq = torch.norm(actions, p=2, dim=-1)  # [seq_len]
            avg_l2_norm = l2_norms_in_seq.mean().item()
            all_l2_norms.append(avg_l2_norm)
    ...
    train_loader = DataLoader(
        ...
        sampler=train_sampler if use_balanced_sampling else None,
        shuffle=False if use_balanced_sampling else True,
    ...
    # Get data dimensions from trajectory dataset
    sample_obs, sample_action, sample_mask = full_dataset[0]
    # For trajectories: obs is [seq_len, channels, H, W], actions is [seq_len, action_dim]
    input_channels = sample_obs.shape[1]  # Channel dimension is at index 1
    action_dim = sample_action.shape[1]   # Action dimension is at index 1
    
    print(f"\n📐 Data Information:")
    print(f"  Observation shape: {sample_obs.shape}")
    print(f"  Input channels: {input_channels}")
    print(f"  Action dimension: {action_dim}")
    print(f"  Sequence length: {sample_obs.shape[0]}")
    
    # Create ResNet-18 + LSTM actor model
    ...
    try:
        sample_input = torch.randn(1, config.sequence_length, input_channels, config.input_crop_size, config.input_crop_size)
...
"""
    ...
    print(f"\n🧪 Testing final model...")
    test_loss, test_mae_metrics, test_error_distribution, test_abs_error_metrics, test_l2_norms, test_mae_per_sample = validate_epoch(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Trajectory-Based Behavior Cloning Training with LSTM and TBPTT"
    )
    
    # Dataset settings
    parser.add_argument("--dataset-path", type=str, default="datasets/sa_dataset_100k",
                       help="Path to SA dataset")
    parser.add_argument("--target-action", type=str, default="sa_incremental_actions",
    ...
    parser.add_argument("--freeze-encoder", action="store_true",
                       help="Freeze encoder weights (fine-tune LSTM and action head only)")
    
    # LSTM-specific settings
    parser.add_argument("--sequence-length", type=int, default=20,
                       help="Length of trajectory sequences (default: 20)")
    parser.add_argument("--sequence-stride", type=int, default=10,
                       help="Stride for sliding window sequences (default: 10)")
    parser.add_argument("--lstm-hidden-dim", type=int, default=256,
                       help="Hidden dimension for LSTM layers (default: 256)")
    parser.add_argument("--lstm-num-layers", type=int, default=1,
                       help="Number of LSTM layers (default: 1)")
    parser.add_argument("--lstm-dropout", type=float, default=0.0,
                       help="Dropout between LSTM layers (only if num_layers > 1)")
    parser.add_argument("--tbptt-chunk-size", type=int, default=None,
                       help="Chunk size for TBPTT (None = full sequence, default: None)")
    
    # Training settings
    parser.add_argument("--batch-size", type=int, default=16,
                       help="Batch size for training (default: 16 for sequences)")
    ...
    parser.add_argument("--grad-clip", type=float, default=1.0,
                       help="Gradient clipping threshold (default: 1.0, important for LSTMs)")
