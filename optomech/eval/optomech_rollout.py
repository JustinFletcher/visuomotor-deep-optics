        ...
        try:
            ...
            if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
                ...
                if is_bc_model:
                    ...
                    # Extract action_dim from action head
                    ...
                    print(f"🔍 BC model - input_channels: {input_channels}, action_dim: {action_dim}")
                else:
                    # SML model
                ...
                if model_arch == "sml_cnn" and 'conv1.weight' in state_dict and not is_bc_model:
...
def create_model_interface(model_path: str, model_type: str, device: torch.device, 
    ...
    Args:
        ...
        model_type: Type of model ("sml", "bc", "rl", "ddpg", "impala", "custom")
        device: PyTorch device
    ...
    if model_type in ["sml", "supervised", "cnn", "resnet"]:
        ...
        return interface
    elif model_type in ["bc", "behavior_cloning", "resnet18actor"]:
        # Use SMLModelInterface since BC models have same interface (observation -> action)
        interface = SMLModelInterface(model_path, device)
        interface.load_model()
        return interface
...
def make_env(env_id: str, env_args: Namespace) -> Callable:
    ...
    def thunk():
        if env_id == "optomech-v1":
            ...
            # and excluding max_episode_steps/rollout_episode_steps (handled by registration)
            env_kwargs = {}
            if env_args is not None:
                for key, value in vars(env_args).items():
                    if value is not None and key not in ['max_episode_steps', 'rollout_episode_steps']:
...
def setup_environment(env_id: str, env_args: Namespace, num_envs: int = 1) -> gym.vector.VectorEnv:
    ...
    """
    # Determine episode steps - prioritize rollout_episode_steps if provided
    episode_steps = getattr(env_args, 'rollout_episode_steps', None) or env_args.max_episode_steps
    if hasattr(env_args, 'rollout_episode_steps') and env_args.rollout_episode_steps is not None:
        print(f"🔧 Using rollout_episode_steps: {episode_steps}")
    else:
        print(f"📏 Using max_episode_steps: {episode_steps}")
    
    # Delete existing registration to ensure max_episode_steps is updated
...
class UniversalRolloutEngine:
    ...
    def _preprocess_observation(self, obs: np.ndarray) -> np.ndarray:
        """
        Preprocess observations to match training data format.
        
        This MUST match the preprocessing used during training, including:
        - Handling VectorEnv batch dimension
        - Complex to magnitude conversion (if log_scale=True)
        - Log-scaling (if log_scale=True)
        - Center cropping to input_crop_size
        
        Args:
            obs: Raw observation from environment 
                 VectorEnv: [num_envs, channels, height, width]
                 Single env: [channels, height, width]
            
        Returns:
            Preprocessed observation matching training format [num_envs, C, H, W] or [C, H, W]
        """
        # Handle VectorEnv batch dimension [num_envs, C, H, W]
        # If we have 4D input from VectorEnv, we need to process each env separately
        if len(obs.shape) == 4:
            # Process each environment's observation separately
            processed_obs = []
            for env_obs in obs:
                processed_env_obs = self._preprocess_single_observation(env_obs)
                processed_obs.append(processed_env_obs)
            return np.array(processed_obs)
        else:
            # Single environment observation
            return self._preprocess_single_observation(obs)
    
    def _preprocess_single_observation(self, obs: np.ndarray) -> np.ndarray:
        """
